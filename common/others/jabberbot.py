...
class JabberBot(object):
    ...
    def connect(self):
        if not self.conn:
            ...
            if authres <> 'sasl':
                self.log(
                    "Warning: unable to perform SASL auth os %s. Old authentication method used!" % self.jid.getDomain())
    ...
    def callback_presence(self, conn, presence):
        ...
        self.debug('Got presence: %s (type: %s, show: %s, status: %s, subscription: %s)' % (
    ...
    def help(self, mess, args):
        ...
        usage = '\n'.join(sorted(
            ['%s: %s' % (name, command.__doc__ or '(undocumented)') for (name, command) in self.commands.items() if
             name != 'help' and not command._jabberbot_hidden]))
