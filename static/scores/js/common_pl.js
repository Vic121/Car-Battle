function activity_point_switch(pts) {
...
$(document).ready(function () {
    pDates();
    $(".activity_point").each(function () {
        $(this).html(activity_point_switch($(this).html()));
    });
    $(".username").hover(
        function () {
            var line = $(this);
            ...
            $("span.username-hover").click(function (event) {
                ...
                $.ajax({
                    ...
                    success: function (msg) {
                        hover_box.html(msg);
                        hover_box.find("span.hover_box_activity_point").html(
                            activity_point_switch(hover_box.find("span.hover_box_activity_point").html())
                        );
...
function distance_of_time_in_words(minutes) {
    ...
    if (minutes < 50) return dli(minutes, 'minutę', 'minuty', 'minut');
    if (minutes < 90) return ('około godzinę');
    if (minutes < 1080) return dli(Math.round(minutes / 60), 'godzinę', 'godziny', 'godzin');
    if (minutes < 1440) return ('jeden dzień');
    if (minutes < 2880) return ('około jednego dnia');
    else {
        days = Math.round(minutes / 1440);
        if (days < 31) return days + ' dni';
        else return 'około ' + dli(Math.round(days / 30), 'miesiąc', 'miesiące', 'miesięcy');
    }
}
