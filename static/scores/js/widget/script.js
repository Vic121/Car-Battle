...
(function (a, b) {
    ...
    d.fn.extend({
        load: function (a, b, c) {
            ...
            var h = this;
            d.ajax({
                url: a, type: g, dataType: "html", data: b, complete: function (a, b, e) {
    ...
    "getBoundingClientRect"in c.documentElement ? d.fn.offset = function (a) {
        ...
        position: function () {
            ...
            var a = this[0], b = this.offsetParent(), c = this.offset(), e = bZ.test(b[0].nodeName) ? {
                top: 0,
                left: 0
            } : b.offset();
...
if ($jQ.browser.webkit) $jQ.browser.kind = 'webkit';
else if ($jQ.browser.opera) $jQ.browser.kind = 'opera';
else if ($jQ.browser.msie) $jQ.browser.kind = 'msie';
else if ($jQ.browser.mozilla) $jQ.browser.kind = 'mozilla';
else $jQ.browser.kind = 'other';
...
var PickScore = function () {
    this.main_window = 'pickscore_module_more';
};
PickScore.prototype = {
    show_msg: function (msg) {
        var obj = $jQ('#' + this.main_window + ' .msg');
        obj.removeClass('hidden');
        obj.html('<p>' + msg + '</p>');
        // setTimeout("$jQ('#'+this.main_window).addClass('hidden');", 5000);
    },
    ...
    load_url: function (url) {
        this.show_msg('LOADING...');
        var that = this;

        $jQ.get(url, {}, function (data) {
            $jQ('#' + that.main_window).html(data['html']);
            setTimeout("pickscore.hide_msg();", 500);
        }, 'jsonp');

    },

    submit_picks: function () {
        var obj = $jQ('#fixtures').find('input');
        var picks = {};
    ...
    switch_table_class: function (id) {
        var obj = $jQ('#' + id);
        if (obj.find('tr.collapsed').length > 0) {
            obj.find('tr.collapsed').removeClass('collapsed').addClass('expanded');
            obj.find('a.btn_expand').removeClass('btn_expand').addClass('btn_collapse');
        }
        else {
            obj.find('tr.expanded').removeClass('expanded').addClass('collapsed');
            obj.find('a.btn_collapse').removeClass('btn_collapse').addClass('btn_expand');
        }
    }
};

window.pickscore = new PickScore();