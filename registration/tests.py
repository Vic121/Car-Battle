...
class RegistrationModelTests(RegistrationTestCase):
    ...
    def test_activation(self):
        ...
        self.failUnlessEqual(RegistrationProfile.objects.activate_user(
            RegistrationProfile.objects.get(user=self.sample_user).activation_key).pk,
                             self.sample_user.pk)
        ...
        self.failIf(RegistrationProfile.objects.activate_user(
            RegistrationProfile.objects.get(user=self.expired_user).activation_key))
        ...
        h = hashlib.sha1()
        h.update('foo')
        self.failIf(RegistrationProfile.objects.activate_user(h.hexdigest()))
...
class RegistrationViewTests(RegistrationTestCase):
    ...
    def test_activation_view(self):
        ...
        h.update('foo')
        response = self.client.get(reverse('registration_activate',
                                           kwargs={'activation_key': h.hexdigest()}))
        self.failIf(response.context['account'])
